...
export type ProfileAccent = 'crimson' | 'blue' | 'teal' | 'gold' | 'purple' | 'green' | 'amber';

const ACCENT_BG: Record<ProfileAccent, string> = {
...
const ACCENT_COLOR: Record<ProfileAccent, string> = {
  crimson: '#ff8a95',
  blue: '#93c5fd',
  teal: '#5eead4',
  gold: '#d4a843',
...
export interface ProfileAccessItem {
  icon: ReactNode;
  text: string;
...
/**
 * Shared "My Profile" page shell used by every producer/operations role
 * (Team Lead, Digitator, Sewout, QC, ...). CS/Designer/Admin keep their own
 * copies since those predate this extraction — behaviorally identical, just
 * not yet migrated.
 */
export function RoleProfilePage({
  ...
  const roleLabel = NAV_CONFIG[user.role]?.label ?? fallbackRoleLabel;
          ...
          <div className="relative flex-shrink-0">
            ...
            {user.is_active && (
              <span
                ...
                style={{ background: 'var(--color-green)', borderColor: 'var(--glass-bg)' }}
        ...
        <div className="flex flex-col gap-3">
          <Panel title="My Access">
            <ul className="space-y-3">
              {accessItems.map(({ icon, text, accent }) => (
...
function DetailRow({
  ...
  return (
    <div className={`flex items-start gap-3 py-3 ${!last ? 'border-b border-glass-border' : ''}`}>
